...
● re.finditer(pattern,
string,flags=0)：扫描整个字符串，并返回字符串中所有匹配pattern的子串组成的迭代器，迭代器的元素是\_sre.SRE\_Match对象。
...
flags=0)：该函数要求整个字符串匹配pattern，如果匹配则返回包含匹配信息的\_sre.SRE\_Match对象，否则返回None。
...
-  most\_common([n]):该方法返回Counter中出现最多的n个元素
...
defaultdict是dict的子类，因此defaultdict也可被当成dict来使用。它与dict最大的区别在于：如果程序试图根据不存在的key来访问dict中对应的value，则会引发KeyError异常，而default则可以提供一个default\_factory属性，该属性所指定的函数负责为不存在的key来生成value。
