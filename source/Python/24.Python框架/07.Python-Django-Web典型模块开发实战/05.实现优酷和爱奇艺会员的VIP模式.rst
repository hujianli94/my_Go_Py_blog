...
最后使用Django REST
framework的权限管理组件，介绍前后端分离的项目中如何使用权限管理。
...
.. image:: ../../../_static/django_quanxian01.png
...
.. image:: ../../../_static/django_quanxian02.png
...
..
   ...
   注意：
   这里为了演示的简单，新建立了一个用户表，而不是使用Django自带的用户表。
...
.. image:: ../../../_static/django_quanxian06.png
...
.. image:: ../../../_static/django_quanxian07.png

我们再次访问\ http://127.0.0.1:8000/login/\ ，然后输入用户名user2，密码pwd2进行登录。

.. image:: ../../../_static/django_quanxian08.png
...
Control，基于角色的访问控制），如图所示，与我们在之前所介绍的权限管理的生命周期不同，在RBAC模式的生命周期中，增加了“角色”这个概念。

RBAC生命周期

.. image:: ../../../_static/django_rbac001.png
...
.. image:: ../../../_static/django_rbac002.png

（7）如图所示，在Rbac下的权限表内增加权限记录。

.. image:: ../../../_static/django_rbac003.png

（8）如图所示，在Rbac下的角色表里增加一条角色记录，角色：人力资源总监，权限：查看用户2。

.. image:: ../../../_static/django_rbac004.png

（9）如图所示，在Rbac下的用户表里新增两条记录，user11给角色设定为“人力资源总监”，user22不设定任何角色。

.. image:: ../../../_static/django_rbac005.png
...
（12）验证user11可以通过权限验证。如图所示，访问\ http://127.0.0.1:8000/login/\ ，然后登录user11，用户名：user11，密码：pwd11。
...
注意：
新建的中间件应该加在编写中间件所使用的中间件的后面。比如我们编写的权限验证中间件中使用到了Session，那么新建的中间件应该加在Session中间件的后面，这样可以确保不会出现错误。

（3）设置白名单。当我们想要换一个用户名登录时，访问：
http://127.0.0.1:8000/login/\ ，会发现在登录之前被要求权限验证。
...
（10）如图所示，使用Postman以post的方式，向：http://127.0.0.1:8000/auth/
，提交数据：
...
（4）运行项目，如图6-43所示，使用Postman以get的方式携带Token，向http://127.0.0.1:8000/common/?token=5dabaf29e3559be4526e16041a1a9fe6
发送网络请求，成功获取到了数据
...
（1）登录user1。如图所示，使用Postman，以post的方式向：http://127.0.0.1:8000/auth/
提交数据：
...
（3）测试user1获取VIP数据资源。如图6-47所示，使用user1的Token访问：http://127.0.0.1:8000/vip/?token=882130f89bed2f9f99372745ee831c9d
，将返回没有通过权限验证的提示信息。
...
使用user2的token访问http://127.0.0.1:8000/vip/?token=ca6ad66339faab2d5cf7f93f042e0caa
vip资源，返回如下信息：
...
使用user3的token访问http://127.0.0.1:8000/svip/?token=1cb102142157d2ab83547a9980381113
vip资源，返回如下信息：
